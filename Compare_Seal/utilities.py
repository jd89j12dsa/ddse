...
def split_dict_and_dump(dictionary, chunk_size, output_directory):
    ...
    for i, chunk_keys in enumerate(chunks):
        ...
        output_file = os.path.join(output_directory, f'data_chunk_{i}.pkl')
        with open(output_file, 'wb') as file:
            joblib.dump(chunk, file)


def merge_dict_from_files(directory):
    merged_dict = {}

    file_list = [file for file in os.listdir(directory) if file.endswith('.pkl')]
    file_list.sort()

    for file_name in file_list:
        file_path = os.path.join(directory, file_name)
        print('read pkl: ', file_path)
        with open(file_path, 'rb') as file:
            chunk = joblib.load(file)
            for key in chunk:
                merged_dict[key] = chunk[key]
