...
relative_path = '../fhipe/fhipe/'
ipe_path = os.path.abspath(os.path.join(current_dir, relative_path))

sys.path.append(ipe_path)
sys.path.append(os.path.abspath('../fhipe/charm'))
...
tbf = dict()
vbf = dict()
random_set = [random.randint(0, 2**32) for i in range(2)]
...
def Ciphertext_Gen_Phase():
	...
	if process_num == 0:
		...
		print('Average Update time cost: ',(total_etc_u/process_num)/total_Uc_len)
...
def Search_Phase(l_del_num,l_del_rate):
	...
	task_search = task_search[0:1] # test representative keyword search
