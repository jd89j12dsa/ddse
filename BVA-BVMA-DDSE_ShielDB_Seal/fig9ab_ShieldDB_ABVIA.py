...
class Attack: 
    ...
    def Group_cluster(self):
        ...
        new_keyword_identifier = [int(i.replace('F','')) for i in new_keyword_identifier]
        Group = []
        subgroup = []
        count = 0
...
if __name__=='__main__': 
    ...
    print('-----------Test ShieldDB Query Recovery Rate-----------')
    ...
    #     f.close()
    for aa in range(len(ParameterT)):
        print("t: ",ParameterT[aa], "recover_in_each_test_group: ", BVA_acc_different_t[aa])
